...
<div>
  ...
  <el-form label-width="100px" ref="ruleForm" :model="formKey" :rules="formRules">
    <el-form-item v-for="(value, key, index) in formData.properties" :key="index" v-bind="value.itemProps">
      <component :is="value.comp" v-model="value.value" v-bind="value.compProps">
        <template v-if="value.children">
          <component v-for="(it, ind) in value.children" :key="ind" :is="it.comp" v-bind="it.compProps"></component>
        </template>
      </component>
    </el-form-item>
    <!-- 提交表单slot -->
